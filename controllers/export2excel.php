...
if (isset($_POST['export'])) {
    ...
    $aprvd = "SELECT No,Date_Received,Requestor,Project,Delta_PN,SAP_PN,Description,QPA,PR_Qty,UoM,Prev_Price,PPV_Type,Current_Vendor,New_Price_1,Currency_1,LT_1,SPQ_1,MOQ_1,Qty2PurchasetoVendor_1,Total_Amt_1,New_Vendor,New_Price_2,Currency_2,LT_2,SPQ_2,MOQ_2,Qty2PurchasetoVendor_2,Total_Amt_2,Purchasing_Recom,QBOM_Unit_Price,V1_VarianceVSQBOM,bc_recomm,Approver_Name_1,Date_Approved_1,Remarks FROM `ppv` WHERE Request_Status='Approved' ";
...
function writeExcel($values)
{
    try {
        ...
        // Get the style of the first row from column B to AJ
        $firstRowStyle = $worksheet->getStyle('B8:AJ8');
